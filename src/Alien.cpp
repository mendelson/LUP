...
Alien::Alien(float x, float y, int nMinions, int cooldown) :
		sp("img/alien.png"), speed(), shootCoolDown() {
	hp = 30;
	...
	if (alienState == RESTING) {
		...
		if (!taskQueue.empty()) {
			...
			if (action.type == Action::MOVE) {
				speed = *(new Point(action.pos.getX() - box.getCenterX(),
						action.pos.getY() - box.getCenterY()));
				Point* ponto = new Point(
						Game::GetInstance().GetDeltaTime() * speed.getX(),
						Game::GetInstance().GetDeltaTime() * speed.getY());
				box.somaPonto(*ponto);
			...
			if (action.type == Action::SHOOT) {
				...
				float minDist;
				float minI = 0;
				Point* aux = new Point(minionArray[0].box.getCenterX(),
						minionArray[0].box.getCenterY());
				minDist = aux->getDist(*pos);
				delete (aux);
				for (unsigned int i = 1; i < minionArray.size(); i++) {
					aux = new Point(minionArray[i].box.getCenterX(),
							minionArray[i].box.getCenterY());
		...
		if (!bull.targetsPlayer) {
			...
			if (IsDead()) {
				...
				StillAnimation* animacao = new StillAnimation(box.getCenterX(),
						box.getCenterY(), rotation, *sp, 0.3 * 4, true);
				...
				for (unsigned int i = 0; i < minionArray.size(); i++) {
					...
					StillAnimation* animacao = new StillAnimation(
							minionArray[i].box.getCenterX(),
							minionArray[i].box.getCenterY(), rotation, *aux,
							0.3 * 4, true);
					Game::GetInstance().GetCurrentState().AddObject(animacao);
				}
