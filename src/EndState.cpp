...
EndState::EndState(StateData stateData) :
		bg(), music(), instruction("font/Call me maybe.ttf", 20, Text::SOLID,
				"Voce venceu!", { 255, 255, 255 }, 0,
				Game::GetInstance().getHeight() - 40) {
	if (stateData.playerVictory) {
		bg.Open("img/win.jpg");
		music.Open("audio/endStateWin.ogg");
		instruction.SetText(
				"Voce venceu! Pressione espaco para jogar denovo ou esc para voltar a tela inicial");
	} else {
		bg.Open("img/lose.jpg");
		music.Open("audio/endStateLose.ogg");
		instruction.SetText(
				"Voce perdeu! Pressione espaco para jogar denovo ou esc para voltar a tela inicial");
	}
}
