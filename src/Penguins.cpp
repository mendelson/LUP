...
Penguins::Penguins(float x, float y) :
		bodySp("img/penguin.png"), cannonSp("img/cubngun.png"), speed(), timer() {
...
void Penguins::Update(float dt) {
	...
	float speedWalk = 20;
	float gravity = 500;
	float floorHeight = Game::GetInstance().GetCurrentState().getTileMapHeight(
			box.getX());
	float proxHeight = Game::GetInstance().GetCurrentState().getTileMapHeight(
			box.getX() + speedWalk);
	float antHeight = Game::GetInstance().GetCurrentState().getTileMapHeight(
			box.getX() - speedWalk);
...
void Penguins::Render() {
	...
	bodySp.Render(box.getX() + Camera::pos.getX(),
			box.getY() + Camera::pos.getY(), angulo);
	angulo = cannonAngle * 180 / 3.141592653;
	cannonSp.Render(box.getX() + Camera::pos.getX(),
			box.getY() + Camera::pos.getY(), angulo);
}
...
void Penguins::Shoot() {
	...
	Bullet* bullet = new Bullet(
			(float) box.getCenterX()
					+ (cannonSp.GetWidth() * cos(cannonAngle) / 2),
			(float) box.getCenterY()
					+ (cannonSp.GetHeight() * sin(cannonAngle) / 2),
			cannonAngle, fspeed, 2000.0, "img/penguinbullet.png", false, 4);
		...
		if (bull.targetsPlayer) {
			...
			if (IsDead()) {
				...
				StillAnimation* animacao = new StillAnimation(box.getCenterX(),
						box.getCenterY(), rotation, *sp, 0.3 * 5, true);
